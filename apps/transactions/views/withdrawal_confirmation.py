...
class IntaSendWebhookSerializer(serializers.Serializer):
    api_ref = serializers.CharField(required=False, allow_blank=True)
...
class IntaSendWebhookView(generics.GenericAPIView):
    serializer_class = IntaSendWebhookSerializer
    ...
    def post(self, request, *args, **kwargs):
        if not self._is_valid_webhook(request):
            ...
            return Response(
                {"error": "invalid webhook signature"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if not mapped_status:
            logger.warning(
                f"Unknown webhook state '{webhook_status}' for transaction {tx.id}"
            )
            return Response(
                {"error": f"Unknown state '{webhook_status}'"},
        ...
        with db_transaction.atomic():
            ...
            if tx.transaction_type == "withdrawal" and mapped_status == "completed":
                ...
                if intasend_wallet_balance is not None:
                    ...
                    system_wallet.balance = Money(
                        Decimal(intasend_wallet_balance), "KES"
                    )
                    system_wallet.save(update_fields=["balance"])
                    logger.info(
                        f"System wallet updated to {intasend_wallet_balance} KES for withdrawal {tx.id}"
                    )
            ...
            # 7️⃣ Handle completed deposits
                ...
                # Credit user wallet
                deposit_amount = (
                    transactions[0].get("amount") if transactions else tx.amount.amount
                )
                wallet = tx.wallet
                ...
                logger.info(
                    f"User wallet credited with {deposit_amount} KES for deposit {tx.id}"
                )
            ...
            # 8️⃣ Retry logic for failed or processing transactions (optional)
                ...
                logger.info(
                    f"Transaction {tx.id} in {mapped_status} state. Retry count: {tx.metadata_info['retry_count']}"
                )

            tx.save()

        return Response(
            {"status": tx.status, "transaction_id": str(tx.id)},
